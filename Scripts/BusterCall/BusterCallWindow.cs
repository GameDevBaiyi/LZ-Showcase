...
{
    class BusterCallWindow : BindWindow<UI_BusterCallWindow>
    {
        int activiId = 0;
        ...
        protected override void OnOpen()
        {
            base.OnOpen();
            activiId = (int)param;
            View.m_close.onClick.Add(Close);
            data = (BusterCallData)ActivityService.Singleton.Data.GetMainInfo(activiId);
            MainCityStateHelper.Singleton.ModeZoom(true);
        ...
        {
            data = (BusterCallData)ActivityService.Singleton.Data.GetMainInfo(activiId);
            if (data == null || data.closeTime <= TimeUtils.currentTick)
            {
        ...
        void InitBtn()
        {
            RedDotManager.Singleton.RegisterRedDot(RedPath.Bustercall + $"/{activiId}/Shop", View.m_btnShop.m_red);
            ...
            View.m_btnJump.onClick.Set(() =>
            {
               var jumpData =  (CommonExchangeShopData)ActivityService.Singleton.Data.GetMainInfo(data.activity);
                if (jumpData != null)
                {
                    TaskHelper.Singleton.OpenWindow(63, UILayer.Popup, false, jumpData.activity);
                }
            });
            //跳过
        ...
        void OnClickHelp()
        {
            var param = new TwoParam<int, int>() { value1 = 4200001, value2 = 4200028 };
        ...
        void OnClickFight()
        {
            ...
            else
            {
                ...
                {
                    value1 = (int)FightType.BusterCall,
        ...
        void OnClickShop()
        {
            ...
            {
                //进入界面关闭红点
                RedDotManager.Singleton.SetRedDotValue(RedPath.Bustercall + $"/{activiId}/Shop", false);
        ...
        private async void PetStationChange(Base.Event obj)
        {
            if (await BusterCallService.Singleton.OnReqFight(activiId))
            {
                if (BusterCallService.Singleton.resBusterCallFight != null && BusterCallService.Singleton.resBusterCallFight.replay != null)
                {
                    ...
                    {
                        FightTools.Singleton.StartBattle(BusterCallService.Singleton.resBusterCallFight.replay, BusterCallService.Singleton.resBusterCallFight, SceneState.BusterCallFight, "lvl_gq_jingjichang");
                    }
                }
            }
        }
